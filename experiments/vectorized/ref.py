...
Description: Train biLSTM model model (with or without Self Attention) 
    and a feature-driven stimulus representation. Reference game experiment.
"""
import dill
import json
import matplotlib
...
from utils.dataloaders.vectorized.reference_load_dataset import load_dataset, construct_stim_reps, construct_y
from experiments.utils import AverageMeter, AccuracyMeter, save_student_checkpoint, reverse, set_seeds
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hidden-dim-l', type=int, default=50,
                        help='hidden dimensions (language) [default: 50]')
    parser.add_argument('--output-dim-l', type=int, default=50,
                        help='output dimensions (language) [default: 50]')
    ...
    parser.add_argument('--hidden-dim-s', type=int, default=50,
        help='hidden representation dimensions (stim) [default: 50]')
    parser.add_argument('--output-dim-s', type=int, default=50,
        help='output representation dimensions (stim) [default: 50]')
    ...
    parser.add_argument('--data', type=str, default='./data/reference/pilot/{}/dataset.tsv',
    ...
    parser.add_argument('--out-dir', type=str, default='/mnt/fs5/schopra/ratchet/lfl/student/reference',
                        help='where to save models [default: /mnt/fs5/schopra/ratchet/lfl/student/reference]')
    parser.add_argument('--cuda', action='store_true', default=False,
                        help='enables CUDA training')
    parser.add_argument('--lemmatized', action='store_true', default=False,
                        help='enables lemmatization')
    ...
    train_data, val_data, test_data, fields, stim_fields = load_dataset(args.data, lemmatized=args.lemmatized)
    ...
    if args.embeddings == "glove":
        fields['message'][1].build_vocab(train_data, vectors="glove.840B.300d")
        torch.save(fields['message'][1], os.path.join(args.out_dir, model_id, 'vocab.pkl'), pickle_module=dill)
    ...
    kwargs['reference_vocab_field'] = fields['message'][1]
    ...
    kwargs.pop('reference_vocab_field')
    if args.cuda:
        ...
        save_student_checkpoint(
            {
                ...
                'languageModelType': 'bilstm',
                'stimModelType': 'featureMLP',
                'kwargs': kwargs
            },
            is_best,
